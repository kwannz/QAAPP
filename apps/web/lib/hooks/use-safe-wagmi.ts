...
const EMPTY_CONNECT = {
  connect: () => {},
  connectAsync: () => Promise.reject(new Error('Web3 not initialized')),
  connectors: [],
  error: null,
  isError: false,
  isIdle: true,
...
const EMPTY_DISCONNECT = {
  disconnect: () => {},
  disconnectAsync: () => Promise.reject(new Error('Web3 not initialized')),
  error: null,
  isError: false,
  isIdle: true,
  isPending: false,
...
export function useSafeWriteContract() {
  ...
  if (!isWeb3Enabled || !isInWagmiProvider()) {
    return {
      writeContract: () => {},
      writeContractAsync: () => Promise.reject(new Error('Web3 not initialized')),
      data: undefined,
      error: null,
      isError: false,
      isIdle: true,

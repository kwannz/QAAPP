...
let NotificationsService = NotificationsService_1 = class NotificationsService {
    ...
    async getNotificationStats(userId) {
        try {
            const [total, unread, read, urgent, high, medium, low, orderUpdates, commissionPayments, systemAlerts] = await Promise.all([
    ...
    async getAdminNotificationStats(filters) {
        try {
            ...
            const [totalSent, emailSent, pushSent, smsSent, delivered, orderUpdates, commissionPayments, systemAlerts, promotions] = await Promise.all([
    ...
    async calculateOpenRate(whereClause) {
        try {
    ...
    async calculateClickRate(whereClause) {
        try {
    ...
    async getDeliveredCount(whereClause, channel) {
        try {
    ...
    async getOpenedCount(whereClause, channel) {
        try {
            return await this.database.notification.count({
                where: {
                    ...whereClause,
    ...
    async getClickedCount(whereClause, channel) {
        try {
    ...
    async calculateTypeSuccessRate(whereClause, typePattern) {
        try {
    ...
    async calculateAverageDeliveryTime(whereClause) {
        try {
    ...
    async calculateRetryRate(whereClause) {
        try {
